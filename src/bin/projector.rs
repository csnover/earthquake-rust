...
fn main() -> AResult<()> {
    ...
    if args.contains("--help") {
        print!(include_str!("projector.usage"), env::args().next().unwrap_or_else(|| "projector".to_string()));
        for (value, &key) in ScriptCode::VARIANTS.iter().enumerate() {
            println!("    {:2}: {}", value, key);
        }
        exit(1);
...
// TODO: Generate the plist instead
#[cfg(target_os = "macos")]
#[link_section = "__TEXT,__info_plist"]
#[used]
static INFO_PLIST: [u8; 894] = *include_bytes!("projector.Info.plist");
