...
impl ProjectorSettings {
    ...
    pub(super) fn parse_mac(version: ProjectorVersion, bits: &[u8]) -> AResult<Self> {
        ...
        assert_eq!(bits[0..=1], [ 0; 2 ]);
        assert_eq!(bits[4..=5], [ 0; 2 ]);
        assert_eq!(bits[8], 0);
        match version {
            ...
            ProjectorVersion::D4 => {
                ...
                // pre-release D4.
                assert_ne!(bits[11] & 4, 0);
            },
            ProjectorVersion::D5 => {
                assert_ne!(bits[6] & 8, 0);
            },
            ProjectorVersion::D6 => {
                assert_eq!(bits[6] & 0x24, 0x24);
            },
        ...
        let cpu = if bits[7] == 0 {
            ...
            MacCPU::from_bits(bits[7])
                .unwrap_or_else(|| panic!("Unknown CPU {}", bits[7]))
        };
    ...
    pub(crate) fn parse_win(version: ProjectorVersion, platform: Platform, bits: &[u8]) -> AResult<Self> {
        ...
        match version {
            ...
            ProjectorVersion::D4 => {
                assert_eq!(bits[1..=3], [ 0; 3 ]);
                // Apparently these are what happen when projector generation
                // silently breaks
                // assert_eq!(bits[6..=11], [ 2, 0, 0, 0, 2, 0 ]);
                assert_eq!(bits[6..=11], [ 0, 0, 0x80, 2, 0xe0, 1 ]);
            },
            ProjectorVersion::D5 => {
                assert_ne!(bits[0] & 0x10, 0);
                assert_eq!(bits[1..=3], [ 0; 3 ]);
                assert_eq!(bits[5..=11], [ 0; 7 ]);
            },
            ProjectorVersion::D6 => {
                assert_ne!(bits[0] & 0x20, 0);
                assert_eq!(bits[5..=11], [ 0; 7 ]);
            },
            ProjectorVersion::D7 => todo!(),
        }
            ...
            per_version: PerVersionSettings::D3(D3Settings {
                ...
                per_platform: D3PlatformSettings::Mac {
                    ...
                    accel_mode:         match bits[10] {
                        ...
                        2 => AccelMode::Frame,
                        3 => AccelMode::Chunk,
                        _ => panic!("Unknown accel mode {}", bits[10]),
                    },
                },
            }),
        })
